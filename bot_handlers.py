...
#ss ဖမ်းခြင်း၊ ai ကို‌ေပး transaction_id,amount,status ထုတ်၊ bank_notification ရယ် vip ‌ေကြးရယ်နဲ့ တိုက်စစ်၊မှန်ရင်‌ vip ‌ေပး၊ မှားရင် manual စစ်
    ...
    async def check_ss(message):
        ...
        if analysis_image == "ALREADY_USED":
            await bot.edit_message_text(user_id, "လူကြီးမင်း၏ငွေလွှဲ‌ေပြစာသည် အသံုးပြုပြီးသား ဖြစ်‌ပါသဖြင့် **VIP ဝင်၍ မရနိုင်ပါ။**", message_id = message.message_id)
            return
            ...
            try:
                ...
                await bot.edit_message_text(user_id, text = "လူကြီးမင်း၏ငွေလွှဲ‌ေပြစာကို **admin မှ စစ်‌ေဆး‌ေနပါပြီ ‌ေခတ္တ‌ေစာင့်ဆိုင်း‌ေပးပါ။**", message_id = message.message_id)
            except Exception as e:
